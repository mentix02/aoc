...
from lib import EngineSchematic, error


def main():
    if len(sys.argv) != 2:
        error(f'usage: {sys.argv[0]} <filename>')

    schematic = EngineSchematic.from_file(Path(sys.argv[1]))

    total = 0

    for row, col in schematic.iter_symbol_positions():
        total += sum(schematic.extract_nums_around(row, col))
